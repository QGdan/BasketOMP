...
1. **订单级商品共现统计**
   ...
   不同购物篮互不依赖。每个线程维护私有的分桶商品对哈希表和商品热度数组，避免在热点循环中写共享状态。

2. **分桶并行归并**

   商品对按稳定哈希映射到固定桶。归并阶段每个 OpenMP 任务独占一个目标桶，目标桶对应全局哈希表中互不重叠的连续槽位，因此不需要锁、`critical` 或原子更新。

3. **商品级邻接表构建**

   使用两遍式并行构图：先归约度数并预分配写入区间，再按固定哈希槽分区无冲突填充，最后按商品并行排序和 Top-N 截断。
...
| `cooccurrence_openmp.c` | 线程私有分桶哈希表、订单动态调度、无锁分桶并行归并、热度归约及分阶段计时 |
...
| `run_bucket_sweep.py` | 自动运行多组桶数，执行跨实验 checksum/质量一致性校验，计算相对基线改善并选出各线程数最佳桶数 |
| `plot_bucket_sweep.py` | 为桶数扫描生成归并耗时、共现流水线、总算法耗时、相对加速和归并占比图 |
...
| `bucket-sweeps/<run_id>/` | 分桶专项实验的比较 CSV、跨配置验证、最佳桶数报告和五类专项图表 |
...
| `bucketed-parallel-merge.md` | 分桶并行归并的数据结构、无锁并发逻辑、审效结果和平台调参方法 |
...
普通哈希表采用开放寻址和线性探测，负载因子达到约 0.70 时扩容。OpenMP 共现阶段进一步使用混合哈希高位选择桶、低位进行桶内探测：每个线程拥有一组私有小哈希表，归并前根据各桶局部条目数上界一次性分配目标容量。

最终 `PairHashMap` 仍是一块连续 `entries[]`，但增加 `bucket_offsets[]` 描述每个桶的独占槽位范围。各桶并行归并后直接形成可查询的最终共现表，不需要再次串行拼接；现有校验、邻接构建和推荐代码仍可遍历同一连续数组。

`--merge-buckets 0` 表示自动选择：约取线程数的 4 倍并向上取 2 的幂，范围为 64–1024；48 线程默认 256 桶。也可显式传入 128、256、384 等值做平台调优。
...
| `--merge-buckets N` | 归并桶数；0 为按线程数自动选择，1 为串行桶基线 | `0` |
...
- `medium-stages.png`；
- `medium-merge-share.png`；
- `medium-cooccur-pipeline.png`。
...
python3 scripts/run_platform_benchmark.py \
  ...
  --max-neighbors 50 --merge-buckets 0 \
  --proc-bind spread --places cores
```

自动测试点为 `1,2,4,8,12,16,24,32,48`。运行器会生成 manifest、原始 CSV、校验报告、汇总 CSV，以及包含归并占比和共现流水线在内的七类图表。正式报告建议分别运行 Full（`--max-neighbors 0`）与 Top-50：前者用于分析并行扩展性，后者用于展示实际性能—质量权衡。完整说明见 `docs/recommendation-quality-and-48thread-experiments.md`。

分桶数调优应保持数据、线程数和 Top-N 不变。推荐使用专项脚本一次完成基线、候选桶数、跨配置正确性校验、排名和绘图：
...
输出目录 `results/bucket-sweeps/<run_id>/` 包含：

- `bucket-comparison.csv`：各线程数、桶数的阶段中位数、相对 1 桶加速、总算法改善率和排名；
- `validation.json`：跨不同实验目录比较共现/推荐 checksum、图规模和八项推荐质量指标；
- `bucket-report.md`：按 `algorithm_ms` 选出的每个线程数最佳桶数；
- `figures/`：归并耗时、共现计算加归并、总算法耗时、相对加速和归并占比五类图。

比较时必须同时查看 `cooccur_compute_ms`、`merge_ms` 和二者之和：桶数过多可能降低归并时间，却因局部小哈希表分散而增加共现计算时间，最终以 `algorithm_ms` 中位数选择配置。实现说明见 `docs/bucketed-parallel-merge.md`。
...
5. `src/cooccurrence_openmp.c`，理解线程私有分桶统计和无锁并行归并；
