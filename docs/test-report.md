...
- 状态：本地实现与审效通过；学校平台 large 正式性能待补跑
- `PairHashMap` 新增连续分桶布局和稳定桶路由，普通串行哈希表接口保持兼容
- 共现阶段使用每线程、每桶私有哈希表；归并阶段按桶 `dynamic,1` 并行调度
- 每个目标桶只有一个写线程，不使用锁、`critical` 或原子更新
- toy/small 覆盖 1/2/4 线程、三种调度和自动/7/64 桶，逐项结果与串行一致
- `test_pair_hash`、`test_cooccurrence`、推荐、评估和集成测试全部通过
- small 结构化冒烟实验生成新字段 `merge_strategy`、`merge_buckets`，验证状态为 pass
- medium Top-50 8线程本地三次中位数：1桶归并189 ms、64桶27 ms、256桶25 ms
- 三组共现校验和完全一致；当前本地 `algorithm_ms` 最优为64桶242 ms，256桶为243 ms

## O9 分桶实验自动化闭环

- 状态：通过
- `run_bucket_sweep.py` 支持桶数基线、1–48线程列表、调度/chunk、线程绑定、随机种子、预热与重复次数透传
- 不同桶数使用独立实验目录，run ID 显式包含 `mbN`，避免结果混淆
- 扫描完成后跨目录核对共现/推荐 checksum、图规模及八项推荐质量指标，不一致时返回非零
- `bucket-comparison.csv` 新增共现流水线耗时、归并占比、相对基线加速、总算法改善率、标准差、变异系数和排名
- `bucket-report.md` 按每个线程数的 `algorithm_ms` 中位数选择最佳桶数，并同时报告波动和归并占比
- 新增五张桶数专项图；通用实验图新增归并占比图和共现流水线图
- Python 脚本语法检查、验证器自测及 PowerShell 脚本解析检查通过
- small 端到端冒烟覆盖1/2线程与1/7桶，共4组配置；跨配置验证状态为pass，并成功生成比较CSV、报告和全部图表